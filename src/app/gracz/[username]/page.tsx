...
export default function GraczPage() {
  ...
  const extraStats = useMemo(() => {
    // 1. Podział meczów na Ekstraklasę i Towarzyskie
    const eksHistory = playerHistory.filter((m: any) => {
      const matchLeague = (m.league || m.category || m.tournamentName || '').toLowerCase();
      const isFriendly = m.tournamentId === 1 || matchLeague.includes('towarzyskie');
      return !isFriendly;
    });

    const friendlyHistoryFromPlayer = playerHistory.filter((m: any) => {
      const matchLeague = (m.league || m.category || m.tournamentName || '').toLowerCase();
      return m.tournamentId === 1 || matchLeague.includes('towarzyskie');
    });

    // 2. Licz statystyki dla Ekstraklasy (do wartości rynkowej)
    ...
    if (selectedTournament === 'Mecze Towarzyskie') {
      ...
      // Statystyki z historii gracza
      friendlyHistoryFromPlayer.forEach((m: any) => {
        stats.matchesCount++;
      ...
      // Uzupełnij o dane z tournamentPlayer jeśli historia jest pusta
      if (stats.matchesCount === 0 && tournamentPlayer) {
      ...
      const allFriendlyRatings: number[] = [];
      friendlyHistoryFromPlayer.forEach((m: any) => {
        const isWinner = (m.playerTeam === m.teamA && m.scoreA > m.scoreB) || (m.playerTeam === m.teamB && m.scoreB > m.scoreA);
        const isDraw = m.scoreA === m.scoreB;
        const result = isWinner ? 'W' : (isDraw ? 'D' : 'L');
        allFriendlyRatings.push(calculateMatchRating({
          minutes: m.minutesPlayed || (m.role === 'starter' ? 90 : 30),
          goals: m.goals?.length || 0,
          assists: m.assists || 0,
          result: result,
      ...
      if (allFriendlyRatings.length === 0 && tournamentPlayer?.matches) {
        tournamentPlayer.matches.forEach((m: any) => {
      ...
      if (allFriendlyRatings.length > 0) {
        stats.avgRating = allFriendlyRatings.reduce((a, b) => a + b, 0) / allFriendlyRatings.length;
      }
    } else {
      stats.matchesCount = eksStats.matches;
      stats.goals = eksStats.goals;
      stats.assists = eksStats.assists;
      stats.redCards = eksStats.redCards;
      stats.avgRating = eksAvgRating;
      
      eksHistory.forEach((m: any) => {
        if (m.role === 'starter') stats.started++;
        stats.minutes += (m.minutesPlayed || (m.role === 'starter' ? 90 : 30));
        m.cards?.forEach((c: any) => {
          if (String(c.type).toLowerCase().includes('yellow')) stats.yellowCards++;
        });
      });
    }
